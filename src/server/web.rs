...
const WINDOWS_RUNNER: &[u8] =
    include_bytes!("../../deploy/downloads/lorehub-runner-windows-x86_64-v0.2.18.msi");
#[cfg(not(feature = "embedded-runner-installers"))]
const WINDOWS_RUNNER: &[u8] = &[];
#[cfg(feature = "embedded-runner-installers")]
...
pub async fn windows_runner() -> Response {
    download(
        WINDOWS_RUNNER,
        "application/x-msi",
        "lorehub-runner-windows-x86_64-v0.2.18.msi",
    )
}
...
mod tests {
    ...
    async fn page_has_strict_browser_headers() {
        ...
        assert_eq!(
            response.headers()[header::CONTENT_DISPOSITION],
            "attachment; filename=\"lorehub-runner-windows-x86_64-v0.2.18.msi\""
        );
